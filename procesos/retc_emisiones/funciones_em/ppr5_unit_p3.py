...
def updateG2Kg_es():
    ...
    listado = db.dbppr_v4_es.find({"em_agua": {'$gt':0}})   
    for emision in listado:
        logging.info("g/año"+str(emision["_id"]))

        #logging.info("id "+ str(emision["_id"])+ " NRA: " + str(emision["nra"]) + " cas " + emision["cas"] + " sustancia " + emision["sustancia"])

        #if(emision["em_agua"] > Decimal128(0)):
        data1 = emision["em_agua"].to_decimal()
        data1 = data1 * decimal.Decimal(0.001)
        #data = Decimal128(emision["em_agua"]*Decimal128("0.001"))
        print(data1)
        #else:
            #print("No")


        agua =              data1  if emision["em_agua"] != 0 else 0
        #aire =              Decimal128(emision["em_aire"]) / Decimal128(1000)  if emision["em_aire"] != 0 else 0
        #suelo =             Decimal128(emision["em_suelo"]) / Decimal128(1000)  if emision["em_suelo"] != 0 else 0
        #reutilizacion =     Decimal128(emision["tr_reutilizacion"]) / Decimal128(1000)  if emision["tr_reutilizacion"] != 0 else 0
        #reciclado =         Decimal128(emision["tr_reciclado"]) / Decimal128(1000)  if emision["tr_reciclado"] != 0 else 0
        #tratamiento =       Decimal128(emision["tr_tratamiento"]) / Decimal128(1000)  if emision["tr_tratamiento"] != 0 else 0
        #coprocesamiento =   Decimal128(emision["tr_coprocesamiento"]) / Decimal128(1000)  if emision["tr_coprocesamiento"] != 0 else 0
        #dispfinal =         Decimal128(emision["tr_dispfinal"]) / Decimal128(1000)  if emision["tr_dispfinal"] != 0 else 0
        #alcantarillado =    Decimal128(emision["tr_alcantarillado"]) / Decimal128(1000)  if emision["tr_alcantarillado"] != 0 else 0
        #incineracion =      Decimal128(emision["tr_incineracion"]) / Decimal128(1000)  if emision["tr_incineracion"] != 0 else 0
        #otros =             Decimal128(emision["tr_otros"]) / Decimal128(1000)  if emision["tr_otros"] != 0 else 0
        

        #logging.info("\tagua: " + str(emision["em_agua"]) + " new: " + str(agua))
